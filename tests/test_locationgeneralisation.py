...
class TestGeneralisation:
    generaliser = SensitiveDataGeneraliser(logger=logging.getLogger("test"))
    ala_lookup = ALASpeciesLookup(logger=logging.getLogger("test"))
    ...
    def test_sensitive_data(self):
        assert (
            normalise_species_name("Lagorchestes asomatus")
    ...
    def test_ala_species_lookup(self):
        ...
        for species, result in zip(
            species_names, self.ala_lookup.get_bulk(species_names)
        ):
